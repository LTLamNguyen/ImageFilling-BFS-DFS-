...
Stack<T>::Stack()
{
	...
	max_items = DEFAULTCAPACITY; // Set default capacity
	num_items = 0; // No items initially 
	items = new T[max_items]; // Allocate memory for items
...
void Stack<T>::Push(const T& item) {
	...
	if (num_items == max_items) {
		Resize(max_items * EXPANSIONFACTOR); // Resize if array is full
	}

	items[num_items++] = item; // Add item to top of stack
}
...
T Stack<T>::Pop() {
	...
	T item = items[--num_items];      // Remove the top element from the stack
	if (num_items < max_items / SHRINKRATE) {
		size_t newCapacity = std::max<int>(max_items / EXPANSIONFACTOR, DEFAULTCAPACITY);
		Resize(newCapacity);
	}
...
void Stack<T>::Resize(size_t n) {
	...
	T* newItems = new T[n]; // Allocate memory for the new array
	for (size_t i = 0; i < num_items; ++i) {
		newItems[i] = items[i]; // Copy existing elements to new array
	}

	delete[] items; // Free the memory of old array

	// Update the member variables with the new array and capacity
	items = newItems;
	max_items = n;
}
